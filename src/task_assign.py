...
def fetch_tasks():
    url = "http://localhost:8081/task/retrieveTasks"
    response = requests.get(url)
    if response.status_code == 200:
        print("Tasks successfully fetched.")
    else:
        print(f"Error fetching tasks: {response.status_code}")

def run_optaplanner():
    # Run the OptaPlanner solution to assign tasks
    os.system("java -jar TaskAssignment.jar")

def display_assignments():
    with open('./task_assignments_output.txt', 'r') as f:
        print(f.read())

def main():
    fetch_tasks()  # Fetch tasks from Trello
    fetch_employee_data()  # Fetch employee data
    run_optaplanner()  # Run OptaPlanner for task assignment
    display_assignments()  # Display the assignments
